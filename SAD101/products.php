...
function getProductImage($name, $category) {
    ...
    if($category == 'coffee' || strpos($name, 'coffee') !== false) {
        ...
        if(strpos($name, 'latte') !== false) {
            ...
            if(strpos($name, 'pea') !== false || strpos($name, 'butterfly') !== false) 
                return 'https://images.unsplash.com/photo-1544787219-7f47ccb76574?w=400&h=400&fit=crop&auto=format';
        ...
        if(strpos($name, 'mocha') !== false) {
            ...
            if(strpos($name, 'iced') !== false) 
                return 'https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=400&h=400&fit=crop&auto=format';
            if(strpos($name, 'delight') !== false) 
                return 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=400&fit=crop&auto=format';
            return 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=400&fit=crop&auto=format';
        }
        
        // ICED COFFEE
        if(strpos($name, 'iced') !== false && strpos($name, 'coffee') !== false) {
            return 'https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=400&h=400&fit=crop&auto=format';
        }
    ...
    if($category == 'pastry' || strpos($name, 'pastry') !== false) {
        ...
        // ROLL FAMILY
        if(strpos($name, 'roll') !== false) {
            if(strpos($name, 'cinnamon') !== false) 
...
if($search_filter) {
    $sql .= " AND (name LIKE '%" . $conn->real_escape_string($search_filter) . "%' OR description LIKE '%" . $conn->real_escape_string($search_filter) . "%')";
}
...
<head>
    ...
    <title><?php echo $category_filter ? ucfirst($category_filter) . ' Menu' : 'Menu'; ?> - Café Online</title>
    ...
    <style>
        ...
        .search-section {
            ...
            padding: 1.5rem;
            margin-bottom: 2rem;
            box-shadow: 0 5px 20px rgba(67, 40, 24, 0.1);
        }
    ...
    <div class="menu-header">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="menu-title">
                        <?php if($category_filter == 'coffee'): ?>
                            ☕ Coffee Menu
                        <?php elseif($category_filter == 'pastry'): ?>
                            🥐 Pastries Menu
                        <?php else: ?>
                            🍽️ Our Menu
                        <?php endif; ?>
                    </h1>
                    <p class="menu-subtitle">Discover our premium selection of freshly brewed coffee and artisanal pastries</p>
                </div>
                <div class="col-md-4 text-end">
                    <img src="<?php echo $category_filter == 'pastry' ? 'https://images.unsplash.com/photo-1555507036-ab794f4afe5e?w=200&h=200&fit=crop' : 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=200&h=200&fit=crop'; ?>" 
                         alt="<?php echo $category_filter ? ucfirst($category_filter) : 'Menu'; ?>" class="img-fluid rounded-circle" style="max-width: 150px; border: 3px solid rgba(255,255,255,0.3);">
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="col-md-8">
                    <form method="GET" class="d-flex gap-2">
                        ...
                        <input type="text" name="search" class="form-control search-box" 
                               placeholder="Search our delicious menu..." value="<?php echo htmlspecialchars($search_filter); ?>">
                        ...
                        <div class="product-card">
                            <div class="image-container">
                                ...
                                <span class="product-category-badge">
                                    <?php echo $product['category'] == 'coffee' ? '☕' : '🥐'; ?> 
                            ...
                            <div class="product-info">
                                ...
                                <?php if(isset($_SESSION['user_id'])): ?>
                                    <form method="post" class="d-flex align-items-center gap-2">
                                        ...
                                        <button type="submit" name="add_to_cart" class="btn order-btn flex-grow-1">
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="no-products">
                        ...
                        <h3 style="color: #432818; margin-top: 1rem;">
                            <?php if($category_filter == 'pastry'): ?>
                                No pastries found
                            <?php elseif($category_filter == 'coffee'): ?>
                                No coffee found
                            <?php else: ?>
                                No products found
                            <?php endif; ?>
                        ...
                        <?php if(isset($_SESSION['role']) && $_SESSION['role'] == 'admin'): ?>
                            <div class="mt-3">
                                <a href="add_sample_products.php" class="add-products-btn me-2">
                                    <i class="bi bi-plus-circle me-2"></i>Add Sample Products
                                </a>
                                <a href="manage_products.php" class="btn search-btn">
                                    <i class="bi bi-gear me-2"></i>Manage Products
                                </a>
                            </div>
                        <?php else: ?>
                            <a href="products.php" class="btn search-btn">
